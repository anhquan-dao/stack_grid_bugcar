...
namespace stack_grid_bugcar{
    ...
    cv::Mat LayerHandler::getLayerIMG(){
    ...
    int LayerHandler::transform_to_baselink(geometry_msgs::TransformStamped tf_3d_msg){
        ...
        if(data_img.rows == 0 || data_img.cols == 0){
        ...
        data_img.convertTo(data_img_8u, CV_8UC1);
        //ROS_INFO_STREAM("data_img_8u: " << data_img_8u.type() << " " << &data_img_8u);
        cv::warpAffine(data_img_8u, data_img_float, tf_2d_mat_2x3, stack_dim, 
    ...
    template<> void LayerHandler::callback<nav_msgs::OccupancyGrid>(const nav_msgs::OccupancyGrid::ConstPtr input_data){
        //std::lock_guard<std::mutex> lg(data_mutex);s
        ...
        cv::Mat occupancy_grid_mat = cv::Mat(input_data->data, true).reshape(1,input_data->info.height);
        cv::Mat occupancy_grid_mat_resize;
        //ROS_INFO_STREAM(&data_img);

        cv::resize(occupancy_grid_mat, data_img, cv::Size(), layer_resolution/stack_resolution, layer_resolution/stack_resolution);
        data_img += 1;
